...
def create_namecard(number, initial, name, nickname, sentence, R, G, B, output_filename):
    number = number.replace('-','')
    color_code = 'RGB({R}, {G}, {B})'.format(R=R, G=G, B=B)
    image = Image.new("RGBA", (pixel(5.2), pixel(8.6)), color="RGB(230, 230, 230)")
    draw = ImageDraw.Draw(image)
    font = ImageFont.truetype('NanumSquareRoundR.ttf', 7)
    font_small = ImageFont.truetype('NanumSquareRoundR.ttf', 5)
    kover = ImageFont.truetype('koverwatch.ttf', 60)
